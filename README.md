...
import { Truck, Briefcase, Calculator, CheckCircle, ArrowRight, RefreshCw, Leaf } from 'lucide-react';

const DealBookApp = () => {
  const [step, setStep] = useState(1);
  const [selections, setSelections] = useState({ persona: '', goal: '' });

  // Dati di simulazione basati sulla strategia "Decision Tree"
  const getRecommendation = () => {
    if (selections.goal === 'green') return {
      ...
      features: ["Pay-per-use", "Servizi Ecosistema Inclusi", "Zero Rischi Residui"]
    };
    if (selections.goal === 'usage') return {
      title: "Operating Lease & Full Service",
      desc: "Massima flessibilità. Paghi per l'utilizzo senza appesantire il bilancio. Ideale per TCO certo.",
      features: ["Canone All-inclusive", "Manutenzione Inclusa", "Nessun rischio rivendita"]
    };
    return {
      title: "Instalment Sale / Leasing Finanziario",
      desc: "La strada classica verso la proprietà. Costruisci equity nel tuo veicolo IVECO.",
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 p-6 flex flex-col items-center justify-center">
      
      {/* Header / Manifesto Area */}
          ...
          {step === 1 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-500">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  <Truck className="w-8 h-8 text-slate-400 group-hover:text-blue-600 mb-3" />
                  <h3 className="font-bold text-lg">Driver / Owner-Operator</h3>
                ...
                <button 
                  ...
                  className="p-6 border-2 border-slate-200 rounded-xl hover:border-blue-500 hover:bg-blue-50 transition-all text-left group"
                >
                  <Briefcase className="w-8 h-8 text-slate-400 group-hover:text-blue-600 mb-3" />
                  <h3 className="font-bold text-lg">Fleet Manager / SME</h3>
              ...
              <div className="space-y-3">
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="bg-slate-100 p-2 rounded-full group-hover:bg-white"><Calculator className="w-5 h-5 text-slate-600" /></div>
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="bg-slate-100 p-2 rounded-full group-hover:bg-white"><RefreshCw className="w-5 h-5 text-slate-600" /></div>
                    <div className="text-left">
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="bg-green-100 p-2 rounded-full group-hover:bg-white"><Leaf className="w-5 h-5 text-green-600" /></div>
                    <div className="text-left">
      ...
      <p className="mt-8 text-slate-400 text-xs">IVECO CAPITAL - Internal Strategic Prototype v1.0</p>
    </div>
  );
};

export default DealBookApp;
